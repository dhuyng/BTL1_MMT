...
class ServerApp:
    def __init__(self, root):
        ...
        # self.tree.column("#2", width=200)  # Đặt kích thước tối thiểu và kích thước thực tế cho cột #2
    ...
    def accept_connections(self):
        while self.running:
            try:
                ...
                # Lưu trạng thái kết nối trong danh sách
                ...
                if data == "GET_DATA":
                    ...
                    # self.tree.item(item, values=(ip, data, data))
                    parts = data.rsplit(":", 1)
                    if len(parts) == 2:
                        path = parts[0]
                        port = parts[1]
    ...
    def is_socket_connected(self, sock):
        try:
            # Thử gửi dữ liệu trống để kiểm tra kết nối
            sock.send(b"")
            return True
    ...
    def is_client_connected(self, ip, port):
        return f"{ip}:{port}" in self.connected_clients
